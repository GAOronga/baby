(function($,undefined){
		$.fn.slider = function(opts){
			...
			Slider.prototype.topBottom = function(){
				this.lists.css({"width":options.width,"height":options.height*this.list.length});				
				this.timer = setInterval(move,options.interval);
				var _this=this;
				var index = 0;
				function move(){
					...
					_this.lists.stop().animate({"top":-index*options.height},1000,function(){
				...
				this.btns.find(".btn").eq(0).click(function(){
					...
					if(index==0){
						index=_this.list.length-3;
						_this.lists.css("top",(-(_this.list.length-1))*options.height);
	...
	$("#banner .margin #sliderBox").slider({showNav:true,showBtns:true,width:1090,height:350,diretion:"left"});